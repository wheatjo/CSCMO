...
algorithm = CSCMO(pop_o_init=pop_init, pop_size=100, n_offspring=10, max_FE=n_eval, output=MyOutput())
...
f = open(os.path.join(save_pickle_dir, 'pickle_file/BiCo', 'cscmo_'+problem_name+'_data.pickle'), 'wb')
pickle.dump(res, f)
# f = open(os.path.join(dir_mytest, 'pickle_file', 'cscmo_' + problem_name +'_data.pickle'), 'rb')
# res = pickle.load(f)

save_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'visual_result/BiCo')
