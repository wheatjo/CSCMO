...
def visualize_process_no_region(pop_history: list, test_problem, problem_name: str, save_path):
    plt.ioff()
    fig, ax = plt.subplots()
    ...
    ll = np.argsort(pf[:, 0])[0]
    lr = np.argsort(pf[:, 0])[-1]
    ul = np.argsort(pf[:, 1])[0]
    ur = np.argsort(pf[:, 1])[-1]
    ...
    def update(frame):
        ...
        pf_data = frame['pf']
        ax.set_xlim(pf_data[ll][0], pf_data[lr][0])
        ax.set_ylim(pf_data[ul][1], pf_data[ur][1])
    ...
    def init():
        ax.set_xlim(pf[ll][0], pf[lr][0])
        ax.set_ylim(pf[ul][1], pf[ur][1])
        return pop_plot
...
def visualize_process_one_pop_no_region(pop_history: list, test_problem, problem_name: str, save_path):
    plt.ioff()
    fig, ax = plt.subplots()
    pop_plot = ax.plot([], [], 'go', markerfacecolor='none', label='pop_o')[0]
    # archive_plot = ax.plot([], [], 'r^', markerfacecolor='none')[0]
    ...
    frame_data = []
    for i, alg in enumerate(pop_history):
        frame_data.append({'gen': i, 'pop_data': alg.pop.get('F'),
                           'opt': alg.opt.get('F'), 'pf': alg.problem.pareto_front(),
                           'n_eval': alg.evaluator.n_eval})
...
def display_result_no_region(test_problem, pop):
